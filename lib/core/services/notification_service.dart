...
Future<void> _sendMonthlySummary() async {
  ...
  final total = all.fold<double>(0, (s, e) => s + e.effectiveAmount(now.month, now.year));
  final paid = all.where((e) => e.isPaidInMonth(now.month, now.year)).fold<double>(0, (s, e) => s + e.effectiveAmount(now.month, now.year));
  final unpaid = total - paid;
  final unpaidCount = all.where((e) => !e.isPaidInMonth(now.month, now.year)).length;
...
Future<void> _sendWeeklyPreview() async {
  ...
  final upcoming = all.where((e) {
    if (e.isPaidInMonth(now.month, now.year) || e.dueDay == null) return false;
  ...
  final total = upcoming.fold<double>(0, (s, e) => s + e.effectiveAmount(now.month, now.year));
...
Future<void> _sendOverdueReminder() async {
  ...
  final total = needsReminder.fold<double>(0, (s, e) => s + e.effectiveAmount(now.month, now.year));
...
Future<void> _sendValueUpdateReminder() async {
  ...
  final all = await repo.getAll();
  final variableExpenses = all.where((e) => e.isVariable).toList();
