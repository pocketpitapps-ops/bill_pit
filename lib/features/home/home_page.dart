    ...
    return SafeArea(
      child: FutureBuilder<List<Expense>>(
        ...
        builder: (context, snapshot) {
          ...
          final grandTotal = expenses.fold<double>(0, (s, e) => s + e.effectiveAmount(_selectedMonth, _selectedYear));
          final monthKey = Expense.monthKey(_selectedMonth, _selectedYear);
          final paidTotal = expenses.where((e) => e.paidMonths.contains(monthKey)).fold<double>(0, (s, e) => s + e.effectiveAmount(_selectedMonth, _selectedYear));
          final unpaidTotal = grandTotal - paidTotal;
          final paidCount = expenses.where((e) => e.paidMonths.contains(monthKey)).length;

          final byCategory = <String, double>{};
          for (final e in expenses) {
            byCategory[e.category] = (byCategory[e.category] ?? 0) + e.effectiveAmount(_selectedMonth, _selectedYear);
          }
                  ...
                  ...upcoming.map((e) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        trailing: Text('${e.effectiveAmount(_selectedMonth, _selectedYear).toStringAsFixed(2)}€', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
                      ),
                    );
                  }),
